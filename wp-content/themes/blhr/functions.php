...
function bootstrapwp_breadcrumbs() {
    ...
    if (!is_home() && !is_front_page() || is_paged()) {
        ...
        if (is_category()) {
            ...
            if ($thisCat->parent != 0) {
                echo '<li>'.get_category_parents($parentCat, true, $sep).'</li>';
            }
             echo $before. __('Archive by category ', 'blhr-theme'). '"'. single_cat_title('', false) . '"' . $after;
            ...
            if (get_post_type() != 'post') {
                ...
                $cat = $cat[0];
                if ($cat != null)
                    echo '<li>' . get_category_parents($cat, true, " <span style=\"font-weight: bold; padding: 0 5px;\">></span> ") . '</li>';
                else
                    echo $before;
            ...
            echo '<li><a href="/firmen">Firmen</a></li> ';
            $companyName = wp_cache_get( 'company-name', 'companies');
            ...
            echo $before . str_replace('%1', get_search_query(), __('Search results for "%1"', 'blhr-theme'))  . $after;
        } elseif (is_tag()) {
            echo $before . __('Posts tagged "', 'blhr-theme') . single_tag_title('', false) . '"' . $after;
        } elseif (is_author()) {
            global $author;
            $userdata = get_userdata($author);
            echo $before . __('Articles posted by ', 'blhr-theme') . $userdata->display_name . $after;
        } elseif (is_404()) {
            echo $before . __('Error 404', 'blhr-theme') . $after;
        }
